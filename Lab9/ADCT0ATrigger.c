...
void ADC0Seq3_Handler(void){
  ADC0_ISC_R = 0x08;          // acknowledge ADC sequence 3 completion
	if(bufferIndex < 120) {
		data[bufferIndex] = ADC0_SSFIFO3_R;  // 12-bit result
		//UART_OutUDec(data[bufferIndex]);
		//UART_OutString("\n\r");	
		if(bufferIndex%50 == 0) {
			//draw(getTemp(data[bufferIndex]));
		}
		bufferIndex = (bufferIndex + 1)%120;
	}
}

void Disable_ADC() {
		SYSCTL_RCGCADC_R &= ~0x01;     // disable ADC
