...
void Buttons_Arm(void) {
	GPIO_PORTE_ICR_R = 0x30;    // clear flag6-4
  GPIO_PORTE_IM_R |= 0x30;    // enable interrupt on PE6-4
...
void Buttons_Init(void(*task)(void)) {
  ...
  SW2 = 0;
  GPIO_PORTE_AMSEL_R &= ~0x30;// disable analog function on PE6-4
  GPIO_PORTE_PCTL_R &= ~0x00FF0000; // configure PE5-4 as GPIO <-- activate PE6???
  GPIO_PORTE_DIR_R &= ~0x30;  // make PE6-4 in 
  GPIO_PORTE_AFSEL_R &= ~0x30;// disable alt funct on PE6-4 
  GPIO_PORTE_DEN_R |= 0x30;   // enable digital I/O on PE6-4
  GPIO_PORTE_IS_R &= ~0x30;   // PE6-4 is edge-sensitive 
  GPIO_PORTE_IBE_R &= ~0x30;  // PE6-4 is not both edges 
  GPIO_PORTE_IEV_R |= 0x30;   // PE6-4 rising edge event
  Buttons_Arm();
}
void GPIOPortE_Handler(void){
	GPIO_PORTE_IM_R &= ~0x3C;     // disarm interrupt on PF4 
