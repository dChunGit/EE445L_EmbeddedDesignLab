...
void ADC0_InitSWTriggerSeq3_Ch9(void){ 
  ...
  SYSCTL_RCGCGPIO_R |= 0x08; //Set up outputs on Port D
  while((SYSCTL_PRGPIO_R&0x08) != 0x08){};
  GPIO_PORTD_DIR_R &= ~0x40;      // 2) make PE1 input
  GPIO_PORTD_AFSEL_R |= 0x40;     // 3) enable alternate function on PE1
  GPIO_PORTD_DEN_R &= ~0x40;      // 4) disable digital I/O on PE1
  GPIO_PORTD_AMSEL_R |= 0x40;     // 5) enable analog functionality on PE1
